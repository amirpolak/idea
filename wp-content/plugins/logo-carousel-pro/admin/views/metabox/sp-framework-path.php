 ...
 *
 */
defined( 'SP_VERSION' ) || define( 'SP_VERSION', '1.0.1' );
defined( 'SP_LCPRO_OPTION' ) || define( 'SP_LCPRO_OPTION', '_sp_lcpro_options' );
defined( 'SP_CUSTOMIZE' ) || define( 'SP_CUSTOMIZE', '_sp_customize_options' );
...
if ( ! function_exists( 'sp_get_path_locate' ) ) {
	function sp_get_path_locate() {
		...
		return apply_filters( 'sp_get_path_locate', array(
...
defined( 'SP_BASENAME' ) || define( 'SP_BASENAME', $get_path['basename'] );
defined( 'SP_DIR' ) || define( 'SP_DIR', $get_path['dir'] );
defined( 'SP_URI' ) || define( 'SP_URI', $get_path['uri'] );
 ...
 *
 */
if ( ! function_exists( 'sp_lcpro_locate_template' ) ) {
	function sp_lcpro_locate_template( $template_name ) {
		...
		$located = apply_filters( 'sp_lcpro_locate_template', $located, $template_name );
 ...
 *
 */
if ( ! function_exists( 'sp_lcpro_get_option' ) ) {
	function sp_lcpro_get_option( $option_name = '', $default = '' ) {

		$options = apply_filters( 'sp_lcpro_get_option', get_option( SP_LCPRO_OPTION ), $option_name, $default );
...
if ( ! function_exists( 'sp_set_option' ) ) {
	function sp_set_option( $option_name = '', $new_value = '' ) {
		...
		$options = apply_filters( 'sp_set_option', get_option( SP_LCPRO_OPTION ), $option_name, $new_value );

		if( ! empty( $option_name ) ) {
			$options[$option_name] = $new_value;
			update_option( SP_LCPRO_OPTION, $options );
		}
...
if ( ! function_exists( 'sp_get_all_option' ) ) {
	function sp_get_all_option() {
		return get_option( SP_LCPRO_OPTION );
	}
}
...
if ( ! function_exists( 'sp_get_multilang_option' ) ) {
	function sp_get_multilang_option( $option_name = '', $default = '' ) {
		...
		$value     = sp_lcpro_get_option( $option_name, $default );

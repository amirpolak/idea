...
$outline     .= '<div class="sp-logo-carousel-pro-section layout-' . $layout . ' sp-lcpro-id-' . $post_id . ' sp-logo-section-id-' . $custom_id . ' nav_position_' . $nav_position . ' title_position_' . $title_position . ' sp-lcpro-clear ' . $vertical_class . '">';
if ( 'true' == $tooltip ) {
...
if ( 'ticker' == $carousel_mode ) {
	$outline .= '<div id="sp-logo-carousel-pro' . $custom_id . '" class="sp-logo-carousel-pro-area' . $preloader_class . ' mode_ticker ' . ( 'true' == $tooltip ? 'splcp-ttip' : '' ) . '" ' . $lcp_bx_option . '>';
} else {
	$outline .= '<div id="sp-logo-carousel-pro' . $custom_id . '" class="sp-logo-carousel-pro-area' . $preloader_class . ' mode_standard  ' . ( 'true' == $tooltip ? 'splcp-ttip' : '' ) . '" ' . $lcp_slick_options . '>';
}
...
if ( $que->have_posts() ) {
	while ( $que->have_posts() ) :
		...
		$outline .= '<div class="sp-lcp-item text-center">';
		if ( 'true' == $tooltip ) {
		...
		if ( has_post_thumbnail() ) {
			if ( $logo_image ) {
				if ( 'true' == $lazy && 'ticker' !== $carousel_mode ) {
					$outline .= '<img class="lazy" src="' . SP_LOGO_CAROUSEL_PRO_URL . 'public/assets/css/images/bx_loader.gif" data-lazy="' . esc_url( $logo_image ) . '" alt="' . $logo_image_alt_title . '"';
				} else {
					$outline .= '<img src="' . esc_url( $logo_image ) . '" alt="' . $logo_image_alt_title . '"';
				}
			} else {
				if ( 'true' == $lazy && 'ticker' !== $carousel_mode ) {
					$outline .= '<img class="lazy" src="' . SP_LOGO_CAROUSEL_PRO_URL . 'public/assets/css/images/bx_loader.gif" data-lazy="' . esc_url( $lcp_img_url ) . '" alt="' . $logo_image_alt_title . '" ';
				} else {
					$outline .= '<img src="' . esc_url( $lcp_img_url ) . '" alt="' . $logo_image_alt_title . '" ';
				}
			}
			$outline .= '>';
		}

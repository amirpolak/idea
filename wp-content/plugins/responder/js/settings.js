...
function UniteSettingsRes(){
	...
	var g_objProvider = new UniteProviderAdminRes();
	...
	if(!g_resAdmin)
		var g_resAdmin = new UniteAdminRes();
	...
	function getObjInputs(controlsOnly){
		...
		if(controlsOnly === true){
			...
			if(g_temp.objItemsManager){
				...
				selectorNot += ", .uc-setting-items-panel *";
			}
			
			//fonts
			if(g_temp.objFontsPanel){
				selectors += ", .uc-setting-fonts-panel";
				selectorNot += ", .uc-setting-fonts-panel *";
			}
		...
		switch(type){
			...
			case "mp3":
				...
				if(source != "addon" && jQuery.isNumeric(value) == false)
					value = g_resAdmin.urlToRelative(value);
			break;
			case "post":
	...
	function clearInput(objInput, dataname, checkboxDataName){
		...
		switch(type){
			...
			case "checkbox":
				...
				defaultValue = g_resAdmin.strToBool(defaultValue);
			...
			break;
			default:
				var success = g_resAdmin.clearProviderSetting(type, objInput, dataname);
				if(success == false){
	...
	function setInputValue(objInput, value, value2){
		...
		switch(type){
			...
			break;
			case "checkbox":
				value = g_resAdmin.strToBool(value);
			...
			case "radio":
				...
				if(radioValue === "true" || radioValue === "false"){
					radioValue = g_resAdmin.strToBool(radioValue);
					value = g_resAdmin.strToBool(value);
				}
			...
			default:
				...
				var success =  g_resAdmin.providerSettingSetValue(type, objInput, value);
	...
	this.setValues = function(objValues){
		...
		jQuery.each(objInputs, function(index, input){
			...
			if(objValues.hasOwnProperty(name)){
				...
				switch(type){
					case "post":
						var postTitle = g_resAdmin.getVal(objValues, name+"_post_title");
						if(postTitle)
							value2 = postTitle;
					break;
				}
	...
	function initColorPicker(){
		...
		if(!g_colorPicker && typeof jQuery.unite_farbtastic != "undefined")
	...
	this.updateMp3FieldState = function(objInput, isEnable){
		...
		if(!isEnable){				//set disabled mode
			...
			g_resAdmin.disableInput(objInput);
			g_resAdmin.disableButton(objButton);
			...
			g_resAdmin.enableInput(objInput);
			g_resAdmin.enableButton(objButton);
		}
	...
	function onChooseMp3Click(){
		...
		if(g_resAdmin.isButtonEnabled(objButton) == false)
			return(true);
		
		var objInput = objButton.siblings(".unite-setting-mp3-input");
		var source = objInput.data("source");
		
		g_resAdmin.openAddMp3Dialog(g_uctext.choose_audio,function(urlMp3){
	...
	function setImagePreview(){
		...
		if(source == "addon"){
			...
			if(!urlFull){
				urlFull = g_resAdmin.urlToFull(url);
				objInput.data("urlfull", urlFull);
			}
			
			url = urlFull;
		}else{
			url = g_resAdmin.urlToFull(url);
		}
	...
	function onChooseImageClick(){
		...
		g_resAdmin.openAddImageDialog(g_uctext.choose_image,function(urlImage, imageID){
	...
	this.updateImageFieldState = function(objInput, isEnable){
		...
		if(!isEnable){				//set disabled mode
			...
			g_resAdmin.disableInput(objInput);
			g_resAdmin.disableButton(objButton);
			g_resAdmin.disableButton(objButtonClear);
			objPreview.hide();
			...
			g_resAdmin.enableInput(objInput);
			g_resAdmin.enableButton(objButton);
			g_resAdmin.enableButton(objButtonClear);
	...
	this.setAccordionMaxHeight = function(bodyHeight){
		...
		var spaceBetween = g_resAdmin.getVal(g_options, "accordion_sap", null);
		if(spaceBetween === null){
			trace(g_options);
			throw new Error("Space between accordion items not set in settings options");
		}
				
		var titleHeight = g_resAdmin.getVal(g_options, "accordion_title_height", null);
	...
	this.iconPicker_addIconsType = function(name, arrIcons, iconsTemplate, optParams){
		...
		var objType = g_resAdmin.getGlobalData(key);
		if(objType)
			return(false);
		...
		g_resAdmin.storeGlobalData(key, params);
	};
	...
	function initMapPicker(objPickerWrapper){
		...
		g_resAdmin.validateDomElement(objButton, "choose map button");
		...
		objButton.click(function(){
			...
			g_resAdmin.openIframeDialog("mappicker", urlParams, dialogOptions, onMapUpdate);
	...
	this.initPostPicker = function(objPickerWrapper){
		...
		objButtonSelect.click(function(){
			...
			g_resAdmin.validateNotEmpty(dialogTitle, "dialog title");
			g_resAdmin.openAddPostDialog(dialogTitle, function(response){
	...
	function onAnimationSettingChange(){
		...
		g_resAdmin.validateDomElement(objParent, "Animation setting parent");
		
		var objDemo = objParent.find(".uc-animation-demo span");
		var animation = objSelect.val();
		
		g_resAdmin.validateDomElement(objDemo, "Animation setting demo");
	...
	function onItemsPanelEditItemsClick(){
		...
		g_resAdmin.validateDomElement(objDialog, "items dialog");
	...
	function __________REPEATER_____________(){}
	 ...
	 * add repeater item
	 */
	function repeaterAddItem(event, objWrapper, objItemValues){
		...
		g_resAdmin.validateDomElement(objItemsWrapper, "items wrapper");
		g_resAdmin.validateDomElement(objSettingsTemplate, "settings template");
		...
		html += "		<a class='unite-repeater-trash unite-repeater-buttondelete uc-tip' title='"+deleteText+"'><i class='fa fa-trash' aria-hidden='true'></i></a>";
		...
		g_resAdmin.validateDomElement(objItemSettingsWrapper, "item settigns wrapper");
		
		var objOptions = objItemSettingsWrapper.data("options");
		var idPrefix = objOptions.id_prefix;
		var newID = idPrefix+"item_"+g_resAdmin.getRandomString(5)+"_";
		html = g_resAdmin.replaceAll(html, idPrefix, newID);
		...
		objItemsWrapper.prepend(objItem);
		
		var objHeadTitle = objItem.find(".unite-repeater-item-head span");
		
		var objItemSettings = new UniteSettingsRes();
		...
		if(isNewItem == false){
			...
			if(objInputGeneratedID){
				var generatedID = g_resAdmin.getRandomString(7);
	...
	this.initFontsPanel = function(objFontsWrapper){
		...
		g_temp.objFontsPanel.find(".uc-fontspanel-toggle").click(function(){
			...
			g_resAdmin.validateDomElement(objSection, "fonts panel section");
	...
	this.getFontsPanelData = function(){
		...
		jQuery.each(objCheckboxes, function(index, checkbox){
			...
			var objSection = jQuery("#" + sectionID);
			g_resAdmin.validateDomElement(objSection, "fonts panel section "+sectionID);
	...
	function setFontPanelData(objInput, arrData){
		...
		jQuery.each(arrData, function(sectionName, objFields){
			...
			g_resAdmin.validateDomElement(objSection, "section: "+sectionID);
	...
	function onControlSettingChange(){
		...
		jQuery.each(arrChildControls, function(childName, objControl){
			...
			var arrParents = g_resAdmin.getVal(g_arrChildrenControls, childName);
			if(arrParents)
	...
	this.triggerKeyupEvent = function(objInput, event, funcChange){
		...
		g_resAdmin.runWithTrashold(funcChange, event, objInput);
	...
	function initInputEvents(objInput, funcChange){
		...
		switch(type){
			case "select":
				if(objInput.hasClass("select2") || objInput.hasClass("unite-select2"))
					objInput.select2();
			break;
			case "color":
			...
			break;
			default:
				g_resAdmin.initProviderSettingEvents(type, objInput);
			break;
		}
	...
	function initGlobalEvents(){
		...
		g_resAdmin.onEvent("update_assets_path", onUpdateAssetsPath);
	...
	function initOptions(){
		...
		jQuery.each(arrOptions, function(index, optionKey){
			g_options[optionKey] = g_resAdmin.getVal(objOptions, optionKey, g_options[optionKey]);
	...
	this.destroy = function(){
		...
		g_resAdmin.offEvent("update_assets_path");

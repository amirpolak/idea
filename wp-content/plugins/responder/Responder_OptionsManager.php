...
class Responder_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'responder'));
        }
        ...
        try{
        	...
        	HelperResponder::validateResponderReady();
        ...
        $state = "";
	 	 ...
	 	 if($this ->user_key == '' || $this ->user_secret == ''){
	        ...
	        elseif(empty($this -> alllists) || $this -> alllists == 'Invalid signature' || $this -> alllists == 'Invalid consumer' || (is_string ($this -> alllists) && strpos($this -> alllists, 'Invalid access token') !== false)){
	            $state = "auth_error";
	        }else{
	            $state = "auth_success";
	        }
        ...
        $urlSettingsPage = GlobalsResponder::$urlAdminSettings;
		
        $urlSettingsTab2 = $urlSettingsPage."#plugin_config-2";
        
        
        // HTML for the page
        $settingsGroup = get_class($this) . '-settings-group';
        ?>
 <h1><?php _e('חיבור לרב מסר', 'responder'); ?></h1>
        <div class="responder-settings-page-wraper">
           
        <?php 
        	if($state != "auth_success")
        		require "tabs/settings_page_text.php"
        ?>
                       
            <form method="post" action="<?php echo $urlSettingsTab2?>">
            <?php settings_fields($settingsGroup); ?>
                <style type="text/css">
                    ...
                    .semi-title{width:49%; display: inline-block;}
                    div#plugin_config-2 {
                        background: #fbfaf5;
                    }
                    #res_log{
...
iframe#res_reg::-webkit-scrollbar { 
	...
	<?php if($state != "init_error"):?>
                
                <table id='res_log' class="plugin-options-table"><tbody>
                <?php
                ...
                ?>
                    <tr><td></td><td>
                        <?php $userkey = get_option('Responder_Plugin_EnterUsername');
$usersecret = get_option('Responder_Plugin_EnterPassword');
		
	$submitValue = "שמירה";
		
	if($state == "auth_success")
		$submitValue = "שמירת שינויים";
   
?>
                <p class="submit">
                    <input type="submit" name="save_cha" class="button-primary"
        ...
        switch($state){
        	...
        	break;
        	case "auth_error":
            	auth_admin_notice__error("", $this->lastError);
        	break;
        	case "init_error":
            	auth_admin_notice__error($errorMessage);
        	break;
        	default:
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'responder');
            case 'false':
                return __('false', 'responder');

            case 'Administrator':
                return __('Administrator', 'responder');
            case 'Editor':
                return __('Editor', 'responder');
            case 'Author':
                return __('Author', 'responder');
            case 'Contributor':
                return __('Contributor', 'responder');
            case 'Subscriber':
                return __('Subscriber', 'responder');
            case 'Anyone':
                return __('Anyone', 'responder');
        }
        return $optionValue;
    }

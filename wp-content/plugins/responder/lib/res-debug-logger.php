 ...
 * @link        2016 Tomer Shaked
 *
 * @since   1.0.1
 */
class res_Debug_Logger
{
	/**
	 * Endpoint Helper to retrieve application wide
	...
	function __construct() {
		...
		$this->log_folder_path = RAV_MESSER_PLUGIN_DIR . '/logs';
		// Check config and if debug is enabled then set the enabled flag to true.
		// $options = get_option('res_plugin_options');
	 ...
	 * @since   0.1
	 */
	function get_res_debug_timestamp() {
	 ...
	 * @since   0.1
	 */
	function get_res_debug_status( $level ) {
	 ...
	 * @since   0.1
	 */
	function get_res_section_break( $section_break ) {
		if ( $section_break ) {
	 ...
	 * @since   0.1
	 */
	function reset_res_log_file( $file_name = '' ) {
		if ( empty( $file_name ) ) {
			$file_name = $this->default_log_file;
		}
		$debug_log_file = $this->log_folder_path.'/'.$file_name;
		$content = $this->get_res_debug_timestamp().$this->log_reset_marker;
	 ...
	 * @since   0.1
	 */
	function append_res_to_file( $content, $file_name ) {
	 ...
	 * @since   0.1
	 */
	function log_res_debug( $message, $level = 0, $debug_enabledd = false, $section_break = false, $file_name = '' ) {
		if ( ! $debug_enabledd ) { return; }

		$content = $this->get_res_debug_timestamp(); // Timestamp.
		$content .= $this->get_res_debug_status( $level ); // Debug status.
		$content .= ' : ';
		$content .= $message . "\n";
		$content .= $this->get_res_section_break( $section_break );
		$this->append_res_to_file( $content, $file_name );
	}
	 ...
	 * @since   0.1
	 */
	function log_res_debug_cron( $message, $level = 0, $section_break = false, $debug_enabledd = false ) {
		if ( ! $debug_enabledd ) { return; }
		$content = $this->get_res_debug_timestamp(); // Timestamp.
		$content .= $this->get_res_debug_status( $level ); // Debug status.
		$content .= ' : ';
		...
		$this->append_res_to_file( $content, $this->default_log_file_cron );
	}
	 ...
	 * @since   0.1
	 */
	static function log_res_debug_st( $message, $level = 0, $section_break = false, $file_name = '' ) {

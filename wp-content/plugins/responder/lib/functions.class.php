...
if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly
	...
	public static function sanitizeVar($var, $type){
		...
		switch($type){
			case UniteFunctionsRes::SANITIZE_ID:
				if(empty($var))
					return("");
			...
			break;
			case UniteFunctionsRes::SANITIZE_KEY:
				$var = sanitize_key($var);
			break;
			case UniteFunctionsRes::SANITIZE_TEXT_FIELD:
				$var = sanitize_text_field($var);
			break;
			case UniteFunctionsRes::SANITIZE_NOTHING:
			break;
			default:
				UniteFunctionsRes::throwError("Wrong sanitize type: " . $type);
			break;
		}
		...
		public static function getPostGetVariable($name,$initVar = "", $sanitizeType){
			...
			$var = self::sanitizeVar($var, $sanitizeType);
		...
		public static function convertStdClassToArray($d){
			...
			if (is_array($d)){
				...
				return array_map(array("UniteFunctionsRes","convertStdClassToArray"), $d);
			} else {
				return $d;
			}
		...
		public static function getDiffArrItems($arr, $arrDefault, $arrMustKeys = array()){
			...
			if(!empty($arrMustKeys))
				$arrMustKeys = UniteFunctionsRes::arrayToAssoc($arrMustKeys);
		...
		public static function assocToArrayKeyValue($assoc, $keyName, $valueName, $firstItem = null){
			...
			foreach($assoc as $item){
				if(!array_key_exists($keyName, $item))
					UniteFunctionsRes::throwError("field: $keyName not found in array");
				
				if(!array_key_exists($valueName, $item))
					UniteFunctionsRes::throwError("field: $valueName not found in array");
		...
		public static function trimArrayItems($arr){
			if(gettype($arr) != "array")
				UniteFunctionsRes::throwError("trimArrayItems error: The type must be array");
		...
		public static function insertToAssocArray($arrItems, $arrNewItems){
			...
			foreach($arrNewItems as $item){
				$insertAfter = UniteFunctionsRes::getVal($item, "insert_after");
		...
		public static function arrayToXML($array, $rootName, $xml = null){
			...
			if($xml === null){
				...
				if($strXML === false)
					UniteFunctionsRes::throwError("Wrong xml output");
		...
		public static function maybeUnserialize($str){
			...
			if(!is_array($arrOutput))
				UniteFunctionsRes::throwError("The unserialized string should be alwayas array type");
		...
		/**
		 * check if string contains some english letters
		 */
		public static function isStringContainsEnglish($string){
		...
		public static function validateObjectMethod($object, $strMethod, $objectName){
			...
			if(method_exists($object, "initByID") == false)
				UniteFunctionsRes::throwError("Object: $objectName don't have method $strMethod");
		...
		public static function mkdirValidate($path, $dirName){
			...
			if(is_dir($path) == false){
				@mkdir($path);
				if(!is_dir($path))
					UniteFunctionsRes::throwError("$dirName path: {$path} could not be created. Please check your permissions");
			}
		...
		public static function writeFile($str, $filepath){
			...
			if(is_array($str))
				UniteFunctionsRes::throwError("write file should accept only string in file: ". $filepath);
		...
		public function downloadImage($filepath, $filename, $mimeType=""){
			...
			if($mimeType == ""){
				$info = UniteFunctionsRes::getPathInfo($filepath);
				$ext = $info["extension"];
				$mimeType = "image/$ext";
			}
		...
		public static function downloadFile($filepath, $filename = null){
			...
			UniteFunctionsRes::validateFilepath($filepath,"export file");

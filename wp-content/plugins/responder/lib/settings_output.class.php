...
if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly
	...
	class UniteSettingsOutputResWork extends HtmlOutputBaseRes{
		 ...
		 */
		public function init(UniteSettingsRes $settings){
			
			if($this->isParent == false)
				UniteFunctionsRes::throwError("The output class must be parent of some other class.");
		...
		protected function validateInited(){
			if(empty($this->settings))
				UniteFunctionsRes::throwError("The output class not inited. Please call init() function with some settings class");
		}
		...
		public function setShowSaps($show = true, $type = null){
			...
			switch($type){
				...
				break;
				default:
					UniteFunctionsRes::throwError("Wrong saps type: $type ");
				break;
			}
		...
		protected function getDefaultAddHtml($setting){
			...
			$defaultValue = UniteFunctionsRes::getVal($setting, "default_value");
			$defaultValue = htmlspecialchars($defaultValue);
			
			//UniteFunctionsRes::showTrace();exit();
			
			$value = UniteFunctionsRes::getVal($setting, "value");
			...
			$addParams = UniteFunctionsRes::getVal($setting, UniteSettingsRes::PARAM_ADDPARAMS);
			if(!empty($addParams))
				$addHtml .= " ".$addParams;
		...
		protected function drawSettingRow_getText($setting){
			...
			//modify text:
			$text = UniteFunctionsRes::getVal($setting, "text", "");
			...
			switch($setting["type"]){
				case UniteSettingsRes::TYPE_CHECKBOX:
		...
		protected function drawSettingRow_getTextStyle($setting){
			...
			//set text style:
			$textStyle = UniteFunctionsRes::getVal($setting, UniteSettingsRes::PARAM_TEXTSTYLE);
		...
		protected function drawSettingRow_getRowStyle($setting){
			...
			if($this->showSaps == true && $this->sapsType == self::SAPS_TYPE_INLINE){
				...
				$sap = UniteFunctionsRes::getVal($setting, "sap");
				$sap = (int)$sap;
		...
		*/
		...
		protected function getInputClassAttr($setting, $defaultClass="", $addClassParam="", $wrapClass = true){
			...
			$class = UniteFunctionsRes::getVal($setting, "class", $defaultClass);
			$classAdd = UniteFunctionsRes::getVal($setting, UniteSettingsRes::PARAM_CLASSADD);
			...
			$isTransparent = UniteFunctionsRes::getVal($setting, UniteSettingsRes::PARAM_MODE_TRANSPARENT);
		...
		protected function drawTextInput($setting) {
			...
			$unit = UniteFunctionsRes::getVal($setting, "unit");
			if(!empty($unit))
			...
			$placeholder = UniteFunctionsRes::getVal($setting, "placeholder", null);
		...
		protected function modifyImageSetting($setting){
			...
			$urlBase = UniteFunctionsRes::getVal($setting, "url_base", null);
		...
		protected function drawImageInput($setting){
			...
			//add source param
			$source = UniteFunctionsRes::getVal($setting, "source");
			if(!empty($source))
		...
		protected function drawIconPickerInput($setting){
			...
			$iconsType = UniteFunctionsRes::getVal($setting, "icons_type");
			if($iconsType)
		...
		protected function drawMapPickerInput($setting){
			...
			$strMapData = UniteFunctionsRes::jsonEncodeForHtmlData($value, "mapdata");
		...
		protected function drawColorPickerInput($setting){	
			...
			$rgb = UniteFunctionsRes::html2rgb($bgcolor);
			$bw = UniteFunctionsRes::yiq($rgb[0],$rgb[1],$rgb[2]);
			$color = "#000000";
		...
		protected function drawRepeaterInput($setting){
			...
			$strData = UniteFunctionsRes::jsonEncodeForHtmlData($value, "itemvalues");
			
			$addItemText = UniteFunctionsRes::getVal($setting, "add_button_text");
			if(empty($addItemText))
				$addItemText = __("Add Item", RESPONDER_TEXTDOMAIN);
			
			//get empty text
			$emptyText = UniteFunctionsRes::getVal($setting, "empty_text");
			
			if(empty($emptyText))
				$emptyText = __("No Items Found", RESPONDER_TEXTDOMAIN);
			
			$objSettingsItems = UniteFunctionsRes::getVal($setting, "settings_items");
			UniteFunctionsRes::validateNotEmpty($objSettingsItems, "settings items");
			
			$emptyTextAddHtml = "";
			if(!empty($value))
				$emptyTextAddHtml = "style='display:none'";
			
			$output = new UniteSettingsOutputWideRes();
			$output->init($objSettingsItems);
			
			//get item title
			$itemTitle = UniteFunctionsRes::getVal($setting, "item_title");
			if(empty($itemTitle))
				$itemTitle = __("Item", RESPONDER_TEXTDOMAIN);
				
			$itemTitle = htmlspecialchars($itemTitle);
			
			//delete button text
			$deleteButtonText = UniteFunctionsRes::getVal($setting, "delete_button_text");
			if(empty($deleteButtonText))
				$deleteButtonText = __("Delete Item",RESPONDER_TEXTDOMAIN);
						
			?>
		      <div id="<?php echo $setting["id"]?>" data-settingtype="repeater" <?php echo $strData?> class="unite-settings-repeater unite-setting-input-object" data-name="<?php echo $setting["name"]?>" data-itemtitle='<?php echo $itemTitle?>' data-deletetext='<?php echo $deleteButtonText?>' >
		      	 ...
		      	 <a class="unite-button-secondary unite-repeater-buttonadd uc-tip" ><?php echo $addItemText?></a>
	      	 	...
	      	 	<!--  
	      	 	<a class="unite-button-secondary unite-repeater-buttondelete"><?php echo $deleteButtonText?></a>
	      	 	-->
		...
		protected function drawInputs($setting){
			...
			switch($setting["type"]){
				case UniteSettingsRes::TYPE_TEXT:
					$this->drawTextInput($setting);
				break;
				case UniteSettingsRes::TYPE_COLOR:
					$this->drawColorPickerInput($setting);
				break;
				case UniteSettingsRes::TYPE_SELECT:
					$this->drawSelectInput($setting);
				break;
				case UniteSettingsRes::TYPE_CHECKBOX:
					$this->drawCheckboxInput($setting);
				break;
				case UniteSettingsRes::TYPE_RADIO:
					$this->drawRadioInput($setting);
				break;
				case UniteSettingsRes::TYPE_TEXTAREA:
					$this->drawTextAreaInput($setting);
				break;
				case UniteSettingsRes::TYPE_IMAGE:
					$this->drawImageInput($setting);
				break;
				case UniteSettingsRes::TYPE_MP3:
					$this->drawMp3Input($setting);
				break;
				case UniteSettingsRes::TYPE_ICON:
					$this->drawIconPickerInput($setting);
				break;
				case UniteSettingsRes::TYPE_MAP:
					$this->drawMapPickerInput($setting);
				break;
				case UniteSettingsRes::TYPE_POST:
					$this->drawPostPickerInput($setting);
				break;
				case UniteSettingsRes::TYPE_EDITOR:
					$this->drawEditorInput($setting);
				break;
				case UniteSettingsRes::TYPE_REPEATER:
					
					$this->drawRepeaterInput($setting);
					
				break;
				case UniteSettingsRes::TYPE_CUSTOM:
					
					$callback = UniteFunctionsRes::getVal($setting, "callback");
					if(!empty($callback)){
						
						call_user_func($callback, $setting);
						
					}else{
						if(method_exists($this,"drawCustomInputs") == false){
							UniteFunctionsRes::throwError("Method don't exists: drawCustomInputs, please override the class");
						}
		...
		protected function drawButtonInput($setting){
			...
			$gotoView = UniteFunctionsRes::getVal($setting, "gotoview");
		...
		protected function drawTextAreaInput($setting){
			...
			$rows = UniteFunctionsRes::getVal($setting, "rows");
			if(!empty($rows))
				$rows = "rows='$rows'";
				
			$cols = UniteFunctionsRes::getVal($setting, "cols");
			if(!empty($cols))
				$cols = "cols='$cols'";
		...
		protected function drawRadioInput($setting){
			...
			$isDisabled = UniteFunctionsRes::getVal($setting, "disabled");
			$isDisabled = UniteFunctionsRes::strToBool($isDisabled);
			$settingName = $setting["name"];
			$defaultValue = UniteFunctionsRes::getVal($setting, "default_value");
			$settingValue = UniteFunctionsRes::getVal($setting, "value");
		...
		protected function drawCheckboxInput($setting){
			...
			$value = UniteFunctionsRes::getVal($setting, "value");
			$value = UniteFunctionsRes::strToBool($value);
			
			if($value == true) 
				$checked = 'checked="checked"';
			
				$textNear = UniteFunctionsRes::getVal($setting, "text_near");
			...
			if(!empty($textNear)){
				...
				if($this->showDescAsTips == true){
					$description = UniteFunctionsRes::getVal($setting, "description");
			...
			$defaultValue = UniteFunctionsRes::getVal($setting, "default_value");
			$defaultValue = UniteFunctionsRes::strToBool($defaultValue);
		...
		protected function drawSelectInput($setting){
			...
			$args = UniteFunctionsRes::getVal($setting, "args");
			...
			$arrItems = UniteFunctionsRes::getVal($setting, "items",array());
			if(empty($arrItems))
				$arrItems = array();
		...
		protected function drawInputAdditions($setting,$showDescription = true){
			...
			if($showDescription === false)
				$description = "";
			$unit = UniteFunctionsRes::getVal($setting, "unit");
			$required = UniteFunctionsRes::getVal($setting, "required");
			$addHtml = UniteFunctionsRes::getVal($setting, UniteSettingsRes::PARAM_ADDTEXT);
		...
		* set form id
		...
		public function setFormID($formID){
			...
			if(isset(self::$arrIDs[$formID]))
				UniteFunctionsRes::throwError("Can't output settings with the same ID: $formID");
			
			self::$arrIDs[$formID] = true;
			
			UniteFunctionsRes::validateNotEmpty($formID, "formID");
		...
		public function drawWrapperStart(){
			...
			UniteFunctionsRes::validateNotEmpty($this->settingsMainClass, "settings main class not found, please use wide, inline or sidebar output");
			
			//get options
			$options = $this->getOptions();
			$strOptions = UniteFunctionsRes::jsonEncodeForHtmlData($options);
			...
			*/
			...
			$addHtml = "";
			if(!empty($controls)){
				$strControls = UniteFunctionsRes::jsonEncodeForHtmlData($controls);
		...
		public function draw($formID, $drawForm = false){
			...
			if(empty($this->settings))
				UniteFunctionsRes::throwError("No settings are inited. Please init the settings in output class");
			...
			if($drawForm == true){
				...
				if(empty($formID))
					UniteFunctionsRes::throwError("The form ID can't be empty. you must provide it");
		...
		*/
		...
		protected function drawSapsTabs(){
			...
			<div class="unite-settings-tabs">
				...
				<?php foreach($arrSaps as $key=>$sap){
					$text = $sap["text"];
					UniteFunctionsRes::validateNotEmpty($text,"sap $key text");
		...
		private function drawSettingsRowByType($setting, $mode){
		    ...
		    switch($setting["type"]){
		        case UniteSettingsRes::TYPE_HR:
		            $this->drawHrRow($setting);
		            break;
		        case UniteSettingsRes::TYPE_STATIC_TEXT:
		...
		private function drawSettings_settings($filterSapID = null, $mode=null, $arrSettings = null){
		    ...
		    foreach($arrSettings as $key=>$setting){
		            ...
		            if(isset($setting[UniteSettingsRes::PARAM_NODRAW]))
		                continue;
		                
		                if($filterSapID !== null){
		                    $sapID = UniteFunctionsRes::getVal($setting, "sap");
		...
		public function drawSettings($filterSapID = null){
			...
			//set special mode
			$mode = "";
			if(count($arrSettings) == 1 && $arrSettings[0]["type"] == UniteSettingsRes::TYPE_EDITOR)
			    $mode = "single_editor";
